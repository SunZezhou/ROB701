...
int main(int argc, char **argv) {
    ...
    if (!fin) {
        cerr << "Please run this program in a directory where you have pose.txt" << endl;
        return 1;
    }

    for (int i = 0; i < 5; i++) {
        boost::format fmt("../../rgbd/%s/%d.%s"); //Image File Format
    ...
    // Calculate the point cloud and concate it together
    ...
    for (int i = 0; i < 5; i++) {
        ...
        for (int v = 0; v < color.rows; v++)
            for (int u = 0; u < color.cols; u++) {
                ...
                if (d == 0) continue; // d == 0 indicates that no measurements were taken
