...
int main(int argc, char **argv) {
    ...
    cv::Ptr<cv::StereoSGBM> sgbm = cv::StereoSGBM::create(
        0, 96, 9, 8 * 9 * 9, 32 * 9 * 9, 1, 63, 10, 100, 32);    // magic number
    ...
    // If you have a slow machine, change the v++ and u++ at the end to v+=2, u+=2
    for (int v = 0; v < left.rows; v++)
        for (int u = 0; u < left.cols; u++) {
            ...
            // Calculate the position of the point according to the stereo camera
            double x = (u - cx) / fx;
            double y = (v - cy) / fy;
